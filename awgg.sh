...
WGCOMMAND=$(which amneziawg)

[ -z $WGCOMMAND ] && echo "Amneziawg is not installed." && exit 1
...
function showConfiguration() {
  ...
  script --flush --quiet /dev/null --command "$WGCOMMAND show $DEVLIST" | while read LINE ; do 
    ...
    if [[ $LINE == *"peer"* ]]; then
      ...
      # Output peer line
      if [ $showifname -ne 0 ]; then
        echoLine "    $LINE" $RICHOUTPUT 0
        echoLine "$curif" $RICHOUTPUT 1
      else
      ...
      if [[ "$LINE" == *"interface"* ]]; then
        curif=`echo "$LINE" | cut -f2 -d' '`
        curifplain=$(printf '%s' "$curif" | sed 's/\x1b\[[0-9]\{0,\}m\{0,1\}\x0f\{0,1\}//g' | tr -d '\r\n')
        echoLine "$LINE" $RICHOUTPUT 1
      else
        if [[ "$LINE" == *"listening port"* ]]; then
          echoLine "  $LINE" $RICHOUTPUT 1
          IPLINE=$(echo $LINE | sed 's/listening port/IP address:;/g')
          echoLine "  ${IPLINE%;*} `ip -c -f inet addr show $curifplain | awk '/inet / {print $2}'`" $RICHOUTPUT 1
        else
          echoLine "  $LINE" $RICHOUTPUT 1
        fi
      fi
